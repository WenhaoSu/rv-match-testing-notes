...
...
```
but it also succeeded in generating the `cat` execution file.

When runing `./cat --help`, for the `cat` program generated by `gcc`, it printed the following usage:
```
...
```
However, when trying to use `./cat` normally, such as `./cat ../AUTHORS`, the `kcc` compiled version first reported following message:
```
